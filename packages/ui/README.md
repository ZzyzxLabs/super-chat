...
Card renderers and chat primitives for [@superchat/core](../core/README.md) +
[@superchat/react](../react/README.md). Ships a self-contained plain-CSS
...
- `cards/` — one renderer per built-in card kind (table, chart, funnel, diff, confirm, …), registered in `cards/index.ts` and validated against `@superchat/core`'s `BUILTIN_CARDS` (`cards/registry-sync.test.ts` keeps the two in sync).
...
- `styles.css` — the token baseline. Import it from source (`@superchat/ui/styles.css`), not from `dist`.

## Install

```bash
pnpm add @superchat/core @superchat/react @superchat/ui
```
...
```bash
pnpm --filter @superchat/ui build       # tsup, emits dist/
pnpm --filter @superchat/ui typecheck   # tsc --noEmit
