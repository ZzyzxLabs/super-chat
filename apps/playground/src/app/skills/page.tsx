...
import { ContextBuilder, scoreSkill, userMessage, type BuiltContext } from "@agentloom/core";
  ...
  return (
    <div className="dev__page">
      ...
      <section className="dev__section" style={{ marginTop: 0 }}>
        ...
        <dl className="al-kv" style={{ marginBottom: 18 }}>
          <div className="al-kv__row">
            <dt>
              <code className="al-mono">always</code>
            </dt>
          ...
          <div className="al-kv__row">
            <dt>
              <code className="al-mono">matched</code>
            </dt>
            <dd style={{ textAlign: "left", maxWidth: "62ch" }}>
          ...
          <div className="al-kv__row">
            <dt>
              <code className="al-mono">manual</code>
            </dt>
            <dd style={{ textAlign: "left", maxWidth: "62ch" }}>
              Never injected. The model sees a one-line index and pulls the body with{" "}
              <code className="al-mono">loadSkill</code> when it decides it needs it.
            </dd>
          </div>
        </dl>
      </section>
      ...
      <section className="dev__section">
        ...
        <p className="dev__section-note">
          ...
          via <code className="al-mono">SkillRegistryOptions.matcher</code> if you want semantic retrieval.
        </p>

        <div className="dev__row">
          <input className="dev__input" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Type a user message…" />
          <button type="button" className="al-btn al-btn--primary" onClick={assemble}>
            Assemble context
          </button>
        </div>
        <div className="dev__row">
          {EXAMPLES.map((e) => (
            <button key={e} type="button" className="al-btn al-btn--ghost al-btn--sm" onClick={() => setQuery(e)}>
            ...
            {scored.map(({ skill, score, reason, sections }) => {
              ...
              return (
                <tr key={skill.id} style={{ opacity: enters ? 1 : 0.5 }}>
                  <td>
                    <code className="al-mono">{skill.id}</code>
                    {sections.length ? <div className="al-muted" style={{ fontSize: 11 }}>+ {sections.map((s) => s.id).join(", ")}</div> : null}
                  </td>
                  <td>
                    <span className={`al-pill${skill.mode === "always" ? " al-pill--positive" : skill.mode === "manual" ? " al-pill--info" : ""}`}>
                      {skill.mode}
                    </span>
                  </td>
                  <td>
                    <div className="dev__score">
                      ...
                      <span className="al-mono">{score.toFixed(1)}</span>
                    </div>
                  </td>
      ...
      {built ? (
        <section className="dev__section">
          ...
          <p className="dev__section-note">
            What the provider would actually receive as <code className="al-mono">instructions</code>, plus the trace
          ...
          <div className="dev__split">
            <div className="dev__spec">
                ...
                <tbody>
                  {built.trace.entries.map((e, i) => (
                    <tr key={i} title={e.detail}>
                      ...
                      <td>
                        <span className={`al-pill al-pill--${e.status === "included" ? "positive" : e.status === "failed" ? "negative" : "warning"}`}>
                          {e.status}
                        </span>
                      </td>
              ...
              <div className="al-panel__row" style={{ marginTop: 10 }}>
                <span className="al-muted">Tools unlocked</span>
                <span className="al-mono">{built.toolNames.length}</span>
              </div>
              <div className="al-panel__row">
                <span className="al-muted">Headroom</span>
                <span className="al-mono">{built.trace.headroom.toLocaleString()} tok</span>
              </div>
            </div>
            <div className="dev__preview">
              <div className="dev__spec-label">system</div>
              <pre className="al-pre dev__code" style={{ maxHeight: 460 }}>
                {built.system}
              </pre>
            </div>
          </div>
